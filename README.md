...
2. **基于Sa-Token的会话管理**：集成Sa-Token框架实现无状态认证，提升认证效率
...
### 💻 技术栈整合能力
- **后端技术栈**：Spring Boot + MyBatis-Plus + MySQL + Sa-Token
...
系统提供基于邮箱/密码和Google OAuth 2.0两种认证方式，底层集成 `Sa-Token` 框架实现统一的权限控制。
...
- **登录**：用户提交邮箱和密码，后端进行凭证校验。成功后，生成包含用户ID、角色等信息的token，并返回给客户端。
- **会话维持**：客户端在后续请求的cookie中携带token，后端通过`Sa-Token`中间件解析并验证token，实现无状态认证和接口权限校验，以提升验证效率。
